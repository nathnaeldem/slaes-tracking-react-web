...
$protectedRoutes = ['addProduct','add_bank_deposit', 'updateProduct', 'updateProductStatus','manageProduct', 'getProducts', 'getProductDetails', 'sellProduct', 'add_spending','add_car_spending','change_password','register_user','create_worker','update_vehicle','get_vehicless','create_vehicle','get_unpaid_workers','get_carwash_spendings',
  ...
  'checkout','orderProduct', 'receiveOrder','getNewTransactions', 'payOrderCredit', 'getProductOrders', 'get_daily_sales','get_daily_sales2','checkout2','delete_sale2', 'get_daily_carwash_transactions', 'delete_sale', 'payTransaction', 'get_dashboard_stats', 'get_product_orders']; // Added delete_sale here
...
case 'getAnalyticsAndReports':
    try {
        ...
        $paymentStmt = $pdo->prepare("
            SELECT 
                tp.id, tp.transaction_id, tp.payment_amount, tp.payment_method,
                tp.bank_name, tp.created_at, t.customer_name
            FROM transaction_payments tp
            JOIN transactions t ON tp.transaction_id = t.id
            WHERE tp.organization_id = :org_id
              AND DATE(tp.created_at) BETWEEN :start_date AND :end_date
        ...
        foreach ($salesData as $sale) {
            ...
            elseif ($pmethod === 'partial') {
                ...
                if ($bank_part > 0) $summary['bank_balance_details'][$bname][] = "(+) " . number_format($bank_part, 2) . " - " . $sale_desc . " (Partial)";
            }
            elseif ($pmethod === 'credit') {
                ...
                if ($paidPortion > 0) {
                    if ($bname && isset($summary['bank_balances'][$bname])) {
                        ...
                        $summary['bank_balance_details'][$bname][] = "(+) " . number_format($paidPortion, 2) . " - " . $sale_desc . " (Initial Credit Payment)";
                    } else {
                        $summary['cash_balance'] += $paidPortion;
                        $summary['cash_balance_details'][] = "(+) " . number_format($paidPortion, 2) . " - " . $sale_desc . " (Initial Credit Payment)";
                    }
                }
            }
        }
        ...
        foreach ($expensesData as $exp) {
            ...
            if ($exp['payment_method'] === 'bank' && isset($summary['bank_balances'][$exp['bank_name']])) {
                ...
                $summary['bank_balance_details'][$exp['bank_name']][] = "(-) " . number_format($amt, 2) . " - " . $desc;
            }
            elseif ($exp['payment_method'] === 'cash') {
                $summary['cash_balance'] -= $amt;
                $summary['cash_balance_details'][] = "(-) " . number_format($amt, 2) . " - " . $desc;
            }
        }
        ...
        foreach ($depositsData as $dep) {
            ...
            if (isset($summary['bank_balances'][$dep['bank_name']])) {
                $summary['bank_balances'][$dep['bank_name']] += $amt;
                $summary['cash_balance']    -= $amt;
                $summary['bank_balance_details'][$dep['bank_name']][] = "(+) " . number_format($amt, 2) . " - Deposit";
                $summary['cash_balance_details'][] = "(-) " . number_format($amt, 2) . " - Deposit to " . $dep['bank_name'];
            }
        }
...
case 'pay_unpaid_amount':
    ...
    $bankName = ($paymentMethod === 'bank') ? ($data['bank_name'] ?? null) : null;
    ...
    try {
        ...
        $stmt = $pdo->prepare("
            UPDATE transactions 
            SET unpaid_amount = unpaid_amount - :amount 
        ...
        if ($stmt->rowCount() === 0) {
            throw new Exception('Transaction not found or no update made');
        }
        ...
        echo json_encode([
            ...
            'new_unpaid' => $pdo->query("SELECT unpaid_amount FROM transactions WHERE id = " . $data['transaction_id'])->fetchColumn()
        ]);
    } catch (Exception $e) {
        $pdo->rollBack();
        http_response_code(500);
...
case 'get_daily_sales2':
    ...
    $paymentMethod = $input['payment_method'] ?? 'cash';
    $bankName = ($paymentMethod === 'bank') ? ($input['bank_name'] ?? null) : null;
    ...
    break;
        ...
        case 'get_product_orders':
            try {
                ...
                http_response_code(500);
                echo json_encode(['success' => false, 'message' => 'Failed to fetch product orders: ' . $e->getMessage()]);
            }
            break;
        ...
        case 'create_worker':
            try {
                ...
                if (empty($name)) {
                    http_response_code(400);
                    echo json_encode(['success' => false, 'message' => 'Worker name is required.']);
                    exit;
                }

                $stmt = $pdo->prepare("INSERT INTO workers (name, organization_id) VALUES (:name, :org_id)");
                $stmt->execute([':name' => $name, ':org_id' => $organizationId]);

                http_response_code(201);
                echo json_encode(['success' => true, 'message' => 'Worker created successfully.']);
            } catch (Exception $e) {
                http_response_code(500);
                echo json_encode(['success' => false, 'message' => 'Failed to create worker: ' . $e->getMessage()]);
            }
            break;

        case 'get_workers':
            try {
                $stmt = $pdo->prepare("SELECT id, name FROM workers WHERE organization_id = :org_id");
                $stmt->execute([':org_id' => $organizationId]);
                $workers = $stmt->fetchAll(PDO::FETCH_ASSOC);
                http_response_code(200);
                echo json_encode(['success' => true, 'workers' => $workers]);
                ...
                http_response_code(500);
                echo json_encode(['success' => false, 'message' => 'Failed to fetch workers: ' . $e->getMessage()]);
            }
            break;
        ...
        case 'delete_worker':
            try {
                ...
                $stmt = $pdo->prepare("DELETE FROM workers WHERE id = :id AND organization_id = :org_id");
                $stmt->execute([':id' => $workerId, ':org_id' => $organizationId]);
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    http_response_code(404);
                    echo json_encode(['success' => false, 'message' => 'Worker not found or you do not have permission to delete.']);
                }
            } catch (Exception $e) {
                http_response_code(500);
                echo json_encode(['success' => false, 'message' => 'Failed to delete worker: ' . $e->getMessage()]);
            }
            break;
